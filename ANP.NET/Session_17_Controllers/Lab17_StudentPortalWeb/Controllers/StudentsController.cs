...
// ============================
// Project still works.
// LAB 17 — Lab ID: 8 | MIN_GPA_EDIT = 2.9 | MAX_YEAR_EDIT = 4
//
// Create() GET is not async because it only returns the empty view and performs
// no asynchronous database operations but the POST overload is async because it
// saves data to the database using asynchronous entity framework methods.
// ==================================================================================================   
...
namespace StudentPortalWeb.Controllers
{
    public class StudentsController : Controller
    {
        ...
        //           PART B
        // ============================
        //Create() GET returns an empty form because we are creating a new object while Edit() GET returns a loaded object because we are modifying an existing row.
        //Try a non-existent id:students/edit/9999
        ...
        //           PART C
        // ============================
        // If change the parameter to student.id : it wouldnt change or edit any data that i submitted on the form because the request id dosent match with the parameter name.
        ...
        public async Task<IActionResult> Edit(int id, Student student)
        {
            ...
            TempData["Message"] = $"{student.FullName} updated successfully";

            //return RedirectToAction("Index");
            return RedirectToAction("AhmedConfirmed",student);
        }
        ...
        //           PART E
        // ============================
        // The POST request is finished and student object that existed during the POST no longer exists.
        //A new GET request starts and the action must load the student again from the database instead of trying to reuse the old object.
